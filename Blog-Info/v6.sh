...
getBlogInfo() {
  ...
  blogEnv=$(echo -e "Hexo: $hexoVersion ${strong}${yellow}|${reset} Node.js: $nodeVersion ${strong}${yellow}|${reset} NPM: $npmVersion ${strong}${yellow}|${reset} Theme: $blogTheme")
  ...
  for format in "JPG" "PNG" "GIF" "WebP" "ICO" "SVG"; do
    ...
    imagesCount+=$(printf "%s %d ${strong}${yellow}|${reset} " "$format" "$count")
    imageSize+=$(printf "%s %s ${strong}${yellow}|${reset} " "$format" "$size")
  done
  # 去除末尾 |
  ...
  assetfiles=$(echo -e "JS: $jsCount, $jsSize ${strong}${yellow}|${reset} CSS: $cssCount, $cssSize ${strong}${yellow}|${reset} Font: $fontCount, $fontSize")
...
nowTime=$(date +%s)
# 单位为秒
if ((nowTime - updatedTime <= "$blogUpdateInterval")); then
  startTime="$(date +%s)"
  asyncCommand "getBlogInfo" "获取信息中"
else
  ...
  while true; do
    ...
    if [[ "$choice" =~ [Yy] ]]; then
      echo
      asyncCommand "overloadBlog" "重载博客中"
      echo -e "\033[2K${yellow}重载博客完成${reset}"
      break
