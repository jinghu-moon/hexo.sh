  ...
  while IFS= read -r -d '' image_file; do
    if [ -f "$image_file" ]; then
      ...
      if [[ " $sameFiles" =~ $(basename "$image_file") ]]; then
        cwebp -q $compressibilityFactor -quiet "$image_file" -o "./$folder/$(date '+%Y.%m.%d')_Compressed/$(basename "$image_file" | cut -d. -f1)_$timeStamp.webp" &
      else
        cwebp -q $compressibilityFactor -quiet "$image_file" -o "./$folder/$(date '+%Y.%m.%d')_Compressed/$(basename "$image_file" | cut -d. -f1).webp" &
